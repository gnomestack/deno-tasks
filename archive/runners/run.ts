        ...
        if (writeError) {
            bus.send(
                new UnhandledErrorMessage(
                    new Error(`Unable to find task file: ${taskFile}`),
                ),
            );
        }
        return false;
    ...
    const resolve = (task: ITask) => {
        if (stack.has(task.id)) {
            bus.send(
                new UnhandledErrorMessage(
                    new Error(
                        `Cycle detected in task dependencies: ${[...stack.values(), task.id].join(" -> ")}`,
                    ),
                ),
            );
            return false;
        ...
        for (const dep of task.deps) {
            ...
            if (!depTask) {
                bus.send(
                    new UnhandledErrorMessage(
                        new Error(
                            `Dependency task '${dep}' not found for task '${task.name}'`,
                        ),
                    ),
                );
                return false;
...
export async function run(
    ...
    if (options.workingDirectory) {
        ...
        if (
            !options.workingDirectory || !await fs.exists(options.workingDirectory)
        ) {
            bus.send(
                new UnhandledErrorMessage(
    ...
    if (options.envFile && Array.isArray(options.envFile)) {
        ...
        for (let i = 0; i < envFiles.length; i++) {
            ...
            if (!await fs.exists(envFile)) {
                bus.send(
                    new UnhandledErrorMessage(
                        `Environment file not found: ${options.envFile}`,
                    ),
                );
                return 1;
            }
    ...
    if (options.env && Array.isArray(options.env)) {
        ...
        for (let i = 0; i < envs.length; i++) {
            ...
            try {
                ...
                bus.send(
                    new UnhandledErrorMessage(
                        `unable to parse environment variable: ${env}. ${e}`,
                    ),
                );
                return 1;
            }
    ...
    const envFile = await Deno.makeTempFileSync({
        prefix: "qtr_env",
        suffix: ".env",
    });
    ...
    try {
        if (
            (topLevelTasks.length === 1 && topLevelTasks[0].deps.length === 0) ||
            options.skipDeps
        ) {
