import { ProtectedValue, SecretGenerator, dotenv, env, fs, homeConfigDir, path, ps } from "../deps.ts";
import { parseWorkflow } from "../yaml/mod.ts";
import { getOrCreateKey, KpDatabase, createCredentials } from "../secrets/mod.ts";
import { handlebars, registerDefault } from "../hbs/mod.ts";
import { getTasks } from "../tasks/task-collection.ts";
...
export async function runYaml(file: string) {
            ...
            case "dotenv":
                {
                    ...
                    {
                        isSops = true;
                        const r = await ps.exec("sops", ["-d", vault.path], { stdout: "piped" });
                    ...
                    if (workflow.secrets)
                    {
                        ...
                        for (const [key, entry] of Object.entries(workflow.secrets))
                        {
                            ...
                            else if (entry?.symbols === true || entry?.symbols === undefined)
                                sg.add(symbols)
            ...
            case "kdbx":
                {
                    ...
                    if (typeof(dbFile) !== "string")
                        dbFile = path.join(homeConfigDir(), "gnomestack", "vaults", "default.kdbx");
    ...
    let model = { secrets: secrets, env: env.toObject() }
